...
download.file(
  url = url,
  destfile = paste("data-raw/data", zip_file, sep = "/"),
  quiet = TRUE,
...
zip_list <- unzip(
  zipfile = paste("data-raw/data", zip_file, sep = "/"),
  list = TRUE
)
...
us_tsca <- cleanventory::read_us_tsca(
  path = paste("data-raw/data", file_name, sep = "/")
)

# Remove temporary files

invisible(file.remove(paste("data-raw/data", zip_file, sep = "/")))
